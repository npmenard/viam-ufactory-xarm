...
type myGripper struct {
	...
	arm arm.Arm
	// xarm is the same object as arm, kept concretely so the gripper can drive the
	// gripper Modbus bus itself instead of round-tripping semantic DoCommands. The
	// arm supplies transport; which registers to write is this component's business.
	xarm *xArm
	...
	detected detectedGripper
	// version is submodelG1 or submodelG2 and selects the grasp protocol:
	// G1 keeps the plain Fn700 position move plus a position-stall poll, G2 uses
	// the FnCxx force block-write plus the status register.
	version string
...
func newGripper(ctx context.Context, deps resource.Dependencies, config resource.Config, logger logging.Logger) (gripper.Gripper, error) {
	...
	// Detection has to happen before the kinematics model is built: G1 and G2
	// report different collision geometry. A gripper whose generation cannot be
	// determined is a hard failure rather than a guess — see
	// resolveStandardGripperVersion.
	version, x, detected, err := resolveStandardGripperVersion(ctx, a, newConf.GripperVersion, logger)
	if err != nil {
		return nil, err
	}

	if newConf.UseURDFs && version == submodelG2 {
	...
	mf, err := newGripperKinematics(ModelNameGripper, newConf, logger, func() ([]spatialmath.Geometry, error) {
		return standardGripperGeometries(version)
	})
	if err != nil {
	...
	if newConf.GripperSpeed != 0 || version == submodelG2 {
...
func (g *myGripper) moveG2(ctx context.Context, goal int) (uint16, error) {
	...
	if err := writeForceControlBlock(ctx, g.xarm, g.speed, g.force, uint32(goal)); err != nil { //nolint:gosec // goal is 0..850.
		return 0, err
	}
...
func (g *myGripper) getStatus(ctx context.Context) (uint16, error) {
	r, err := g.xarm.readGripperRegisters(ctx, standardGripperStatusReg, 1)
	if err != nil {
		return 0, err
	}
	if r.exception != 0 {
...
func (g *myGripper) goToPosition(ctx context.Context, goal int) (int, error) {
	...
	if err := g.xarm.setupGripper(ctx); err != nil {
		return 0, err
	}
	if err := g.xarm.setGripperPosition(ctx, uint32(goal)); err != nil { //nolint:gosec // goal is 0..850.
		return 0, err
	}
...
func (g *myGripper) getPosition(ctx context.Context) (int, error) {
	pos, err := g.xarm.getGripperPosition(ctx)
	return int(pos), err
}
...
func (g *myGripper) Geometries(ctx context.Context, _ map[string]any) ([]spatialmath.Geometry, error) {
	...
	return standardGripperGeometries(g.version)
}
